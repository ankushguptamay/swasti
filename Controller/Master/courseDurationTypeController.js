...
const { courseDurationTypeValidation } = require('../../Middleware/Validate/validateMaster');
...
exports.createCourseDurationType = async (req, res) => {
    try {
        ...
        // Check duplicacy
        const { courseDuration, courseType } = req.body;
...
exports.getAllCourseByType = async (req, res) => {
    try {
        ...
        const relation = await Course_Duration_Type.findAll({ where: { courseType: type } });
        // Final Response
        res.status(200).send({
            success: true,
            message: "Course relation fetched successfully!",
